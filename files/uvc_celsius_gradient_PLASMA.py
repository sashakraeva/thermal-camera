...
def apply_thermal_color(data, min_abs_temp=-2, max_abs_temp=50):
    ...
    img_color = cv2.applyColorMap(img_gray, cv2.COLORMAP_PLASMA)
...
def add_colorbar(img, min_abs_temp=-50, max_abs_temp=100):
    ...
    colorbar = cv2.applyColorMap(gradient, cv2.COLORMAP_PLASMA)
    ...
    cv2.putText(img_with_bar, f"{max_abs_temp:.1f}C", (width + 10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 15, 192), 1)
    cv2.putText(img_with_bar, f"{min_abs_temp:.1f}C", (width + 10, height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 225, 0), 1)
